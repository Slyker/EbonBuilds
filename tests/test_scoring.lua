...
function TestScoreBreakdown.setUp()
    settings = EbonBuilds.Build.DefaultSettings()
end

function TestScoreBreakdown.testBreakdownWeightOnly()
    local entry = makeEntry("Fireball", 0, { "Caster DPS" })
    local parts, total = EbonBuilds.Scoring.ScoreBreakdown(entry, 100, settings)
    assertEquals(total, 100)
    assertEquals(#parts, 1)
    assertEquals(parts[1].label, "Weight")
    assertEquals(parts[1].value, 100)
end

function TestScoreBreakdown.testBreakdownWithQualityBonus()
    settings.qualityBonus[2] = 20
    settings.qualityBonusMode[2] = false
    local entry = makeEntry("Fireball", 2, { "Caster DPS" })
    local parts, total = EbonBuilds.Scoring.ScoreBreakdown(entry, 100, settings)
    ...
    assertEquals(parts[2].label, "Rare")
    assertEquals(parts[2].value, "+20")
end

function TestScoreBreakdown.testBreakdownWithQualityBonusMultiplicative()
    settings.qualityBonus[2] = 1.5
    settings.qualityBonusMode[2] = true
    local entry = makeEntry("Fireball", 2, { "Caster DPS" })
    local parts, total = EbonBuilds.Scoring.ScoreBreakdown(entry, 100, settings)
    assertEquals(total, 150)
    assertStrContains(parts[2].value, "x")
end

function TestScoreBreakdown.testBreakdownWithFamilyBonus()
    settings.familyBonus["Caster"] = 30
    settings.familyBonusMode["Caster"] = false
    local entry = makeEntry("Fireball", 0, { "Caster DPS" })
    local parts, total = EbonBuilds.Scoring.ScoreBreakdown(entry, 100, settings)
    assertEquals(total, 130)
    local foundFamily = false
    for _, p in ipairs(parts) do
        if p.label:find("Family") then foundFamily = true end
    end
    assertTrue(foundFamily)
end

function TestScoreBreakdown.testBreakdownWithNovelty()
    settings.noveltyValue = 50
    settings.noveltyMode = false
    local entry = makeEntry("Fireball", 0, { "Caster DPS" })
    local parts, total = EbonBuilds.Scoring.ScoreBreakdown(entry, 100, settings)
    assertEquals(total, 150)
    local foundNovelty = false
    for _, p in ipairs(parts) do
        if p.label == "Novelty" then foundNovelty = true end
    end
    assertTrue(foundNovelty)
end

function TestScoreBreakdown.testBreakdownZeroWeightNoQuality()
    settings.qualityBonus[0] = 0
    settings.noveltyValue = 0
    local entry = makeEntry("Fireball", 0, {})
    local parts, total = EbonBuilds.Scoring.ScoreBreakdown(entry, 0, settings)
    assertEquals(total, 0)
    assertEquals(#parts, 1)
end

function TestScoreBreakdown.testBreakdownWhitelistFiltersFamilies()
    settings.familyBonus["Caster"] = 30
    settings.familyBonus["Melee"] = 10
    settings.familyBonusMode["Caster"] = false
    settings.familyBonusMode["Melee"] = false
    ...
    local parts, total = EbonBuilds.Scoring.ScoreBreakdown(entry, 100, settings)
    assertEquals(total, 130)
    local meleeFound = false
    for _, p in ipairs(parts) do
        if p.label:find("Melee") then meleeFound = true end
    end
    assertFalse(meleeFound)
end
