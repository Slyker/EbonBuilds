...
if not _G.EbonBuilds.Weights then
    _G.EbonBuilds.Weights = { Get = function() return 0 end, Set = function() end }
end
if not _G.EbonBuilds.Toast then
    _G.EbonBuilds.Toast = { ShowAutomationResult = function() end }
end
if not _G.EbonBuilds.Session then
    _G.EbonBuilds.Session = { LogAction = function() end, Init = function() end }
end
if not _G.EbonBuilds.SessionHistory then
    _G.EbonBuilds.SessionHistory = { Init = function() end }
end
if not _G.EbonBuilds.BuildForm then
    _G.EbonBuilds.BuildForm = nil
end
if not _G.EbonBuilds.ViewRouter then
    _G.EbonBuilds.ViewRouter = nil
end

-- 4. Load addon source modules in dependency order
dofile(addonPath("modules/data/Constants.lua"))
