...
local function BuildExportData(build)
	...
	return {
		...
		allClasses = build.allClasses or false,
...
local UH = EbonBuilds.UIHelpers

local importDialog

local function CreateImportDialog()
	local f = UH.CreateDialogFrame(UIParent, 700, 420, "Import Build")
	f:SetName("EbonBuildsImportBuildDialog")
	...
	local _, box = UH.CreateScrollableEditBox(f, f._titleStr, import)
	f._editBox = box
...
function EbonBuilds.ExportImport.ShowImportDialog()
	if not importDialog then importDialog = CreateImportDialog() end
