...
local function EnsureSettings(build)
    if build._settingsEnsured then return end
    build._settingsEnsured = true
...
function EbonBuilds.Build.GetServerLockedPerks()
    if not ProjectEbonhold or not ProjectEbonhold.PerkService then return {} end
    if not ProjectEbonhold.PerkService.GetLockedPerks then return {} end
    return ProjectEbonhold.PerkService.GetLockedPerks() or {}
end

function EbonBuilds.Build.AutoDetectLockedSlots()
    local serverLocked = EbonBuilds.Build.GetServerLockedPerks()
    if #serverLocked == 0 then return 0 end

    local current = EbonBuilds.Scoring.GetEffectiveLockedEchoes()
    local filled = {}
    for i = 1, EbonBuilds.Build.LOCKED_SLOTS do
        if current[i] then
            filled[current[i]] = true
        end
    end
    ...
    return #serverLocked, filled_count
end
